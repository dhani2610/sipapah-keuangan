...
<div class="modal fade" id="modaledit{{ $item2->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">
        ...
        <form role="form text-left" method="POST" action="{{ route('update-catatan-keuangan',$item2->id) }}" enctype="multipart/form-data">
            @csrf
        <div class="modal-body">
                @csrf
                  <div class="mb-3">
                    <label for="">Keterangan</label>
                  <input type="text" class="form-control" placeholder="keterangan" name="keterangan" id="keterangan" value="{{ old('keterangan') ?? $item2->keterangan }}" required>
                  @error('keterangan')
                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                  @enderror
                </div>
                ...
                <div class="mb-3">
                    <label for="">Saldo</label>
                  <input type="number" class="form-control" placeholder="Saldo" name="saldo" id="saldo" value="{{ old('saldo') ?? $item2->saldo }}" required>
                  @error('saldo')
                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                  @enderror
                </div>
                ...
                <div class="mb-3">
                  <label for="">Jenis</label>
                  <select name="jenis" class="form-control" id="" required>
                      <option value="Debet" {{ $item2->jenis == 'Debet' ? 'selected' : '' }}>Debet</option>
                      <option value="Kredit" {{ $item2->jenis == 'Kredit' ? 'selected' : '' }}>Kredit</option>
                  </select>
                @error('jenis')
                  <p class="text-danger text-xs mt-2">{{ $message }}</p>
                @enderror
                ...
                <div class="mb-3">
                  <label for="">Tipe</label>
                  <select name="tipe" class="form-control" id="" required>
                      <option value="Piutang" {{ $item2->jenis == 'Piutang' ? 'selected' : '' }}>Piutang</option>
                      <option value="Utang" {{ $item2->jenis == 'Utang' ? 'selected' : '' }}>Utang</option>
                      <option value="Pemasukan" {{ $item2->jenis == 'Pemasukan' ? 'selected' : '' }}>Pemasukan</option>
                      <option value="Pengeluaran" {{ $item2->jenis == 'Pengeluaran' ? 'selected' : '' }}>Pengeluaran</option>
                  </select>
                @error('tipe')
                  <p class="text-danger text-xs mt-2">{{ $message }}</p>
                @enderror
